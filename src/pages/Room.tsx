...
const Room = () => {
  ...
  const carouselList = [
    { id: 1, img: 'https://raw.githubusercontent.com/hexschool/2022-web-layout-training/main/typescript-hotel/%E6%A1%8C%E6%A9%9F%E7%89%88/banner.png' },
    ...
    <>
      ...
      <div className='bg_primary_10'>
        <div className="z-2 position-relative">
          ...
          {/* banner 輪播區塊 */}
          <div id="carouselExampleIndicators" className="m-0 carousel slide carousel-fade" data-bs-ride="carousel">
            <div className="carousel-indicators mx-0 z-3">
              {carouselList.map((item, index) => (
                <button
                  ...
                  aria-current={index === carouselActiveIndex ? 'true' : 'false'}
            ...
            <div className="carousel-inner rounded-start h-100">
              {
                carouselList.map((item, index) => {
                  return (<div key={item.id} className={`carousel-item ${index === carouselActiveIndex ? 'active' : ''} h-100`}>
        ...
        <div className='container pt-12 pb-14'>
          ...
          <div className='row'>
            {roomList.map((item, roomIndex) => (
              <div className=' bg-white rounded-end d-flex my-5 px-0' key={roomIndex}>
                {/* 輪播區塊 */}
                <div id={`carouselExampleIndicators-${roomIndex}`} className="col-7 carousel slide carousel-fade" data-bs-ride="carousel">
                  <div className="carousel-indicators mx-0">
                    {item.imageUrlList.map((imageUrl, i) => (
                      <button
                        ...
                        data-bs-slide-to={i}
                        className={i === activeIndices[roomIndex] ? 'active' : ''}
                        aria-current={i === activeIndices[roomIndex] ? 'true' : 'false'}
                  ...
                  <div className="carousel-inner rounded-start h-100">
                    {
                      item.imageUrlList.map((imageUrl, i) => {
                        return (<div key={i + 1} className={`carousel-item ${i === activeIndex ? 'active' : ''} h-100`}>
                          <figure className="ratio ratio-16x9 overflow-hidden mb-0 h-100">
                            <img src={imageUrl} className="d-block" alt={`Slide ${i + 1}`} />
                          </figure>
                        </div>);
                      })
                    }
                  </div>
                  <button className="carousel-control-prev" type="button" data-bs-target={`#carouselExampleIndicators-${roomIndex}`} data-bs-slide="prev">
                    <span className="carousel-control-prev-icon" aria-hidden="true"></span>
                    <span className="visually-hidden">Previous</span>
                  </button>
                  <button className="carousel-control-next" type="button" data-bs-target={`#carouselExampleIndicators-${roomIndex}`} data-bs-slide="next">
